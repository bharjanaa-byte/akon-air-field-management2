...
export async function importIntegration(integration) {
  ...
  if (!message || message.id === integration.last_message_id) return { imported: 0, skipped: true, message: message ? 'No newer Master Dispatch email was found.' : 'No matching Master Dispatch email was found.' };
  ...
  const { data: existing, error: readError } = await client.from('jobs').select('work_order').eq('company_id', integration.company_id).in('work_order', workOrders);
  if (readError) throw readError;
  const known = new Set((existing || []).map(job => job.work_order));
  ...
  return { imported: fresh.length, skipped: false, message: fresh.length ? `Imported ${fresh.length} new job${fresh.length === 1 ? '' : 's'}.` : 'This Master Dispatch was already in your jobs list.' };
}
